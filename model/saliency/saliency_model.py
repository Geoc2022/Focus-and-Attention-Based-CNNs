...
def harris_corner_detection_coords(images, k):
    all_keypoints = []

    for image in images:
        channels, height, width = image.shape
        
        np_image = image.cpu().squeeze().numpy()
        uint8_image = np.uint8(np_image * 255)
        
        image_corners = cv2.cornerHarris(uint8_image, 2, 5, 0.05)
        max_corner = image_corners.max()
        corner_threshholding = (image_corners > 0.05 * max_corner)
        valid_corner_coordinates = np.column_stack(np.where(corner_threshholding))
        valid_corner_coordinates = valid_corner_coordinates[:, [1, 0]]

        if len(valid_corner_coordinates) < k:
            corner_values = image_corners[valid_corner_coordinates[:, 1], valid_corner_coordinates[:, 0]]
            sorted_highest_corner_coordinates = np.argsort(-corner_values)
        else:
            sorted_highest_corner_coordinates = np.argsort(-corner_values)[:k]
        keypoints = valid_corner_coordinates[sorted_highest_corner_coordinates]

        keypoints = keypoints.astype(np.float32)
        keypoints[:, 0] = (keypoints[:, 0] / (width - 1)) * 2 - 1
        keypoints[:, 1] = (keypoints[:, 1] / (height - 1)) * 2 - 1

        all_keypoints.append(torch.tensor(keypoints).to(images.device))
