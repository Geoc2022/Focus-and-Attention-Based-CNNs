...
def main():
    ...
    model = focus_sizing_model.KeypointPatchModel(k=k_points, patch_size=patch_size, image_size=image_size).to(device)
    optimizer = optim.Adadelta(model.parameters(), lr=args.lr)
    criterion = nn.CrossEntropyLoss()

    scheduler = StepLR(optimizer, step_size=1, gamma=args.gamma)
    for epoch in range(1, args.epochs + 1):
        focus_sizing_model.train(args, model, device, train_loader, optimizer, criterion, epoch)
        focus_sizing_model.test(model, device, test_loader, criterion)
        scheduler.step()
    ...
    if args.show_patches:
        while input("Show patches and keypoints? (y/n): ").lower() != "n":
            focus_sizing_model.show_patches_and_keypoints(model, device, test_loader)
